 ...
 * @return bool
 */
function ms_categorized_blog() {
	if ( false === ( $all_the_cool_cats = get_transient( 'ms_categories' ) ) ) {
		...
		set_transient( 'ms_categories', $all_the_cool_cats );
	}

	if ( $all_the_cool_cats > 1 ) {
		// This blog has more than 1 category so ms_categorized_blog should return true.
		return true;
	} else {
		// This blog has only 1 category so ms_categorized_blog should return false.
		return false;
	}
}
 ...
 */
function ms_get_attachment_id_from_url( $attachment_url = '' ) {
 ...
 * @return string
 */
function ms_get_placeholder_image( $args = array() ) {
	$default_args = array(
		'width'				=> '',
		'height'			=> '',
	...
	if ( ! is_numeric( $width ) ) {
		throw new Exception( __( 'Width must be an integer', 'ms-inc' ) );
	}

	if ( ! is_numeric( $height ) ) {
		throw new Exception( __( 'Height must be an integer', 'ms-inc' ) );
	}

	if ( ! ctype_xdigit( $background_color ) ) {
		throw new Exception( __( 'Please provide a valid hex color value for background_color', 'ms-inc' ) );
	}

	if ( ! ctype_xdigit( $text_color ) ) {
		throw new Exception( __( 'Please provide a valid hex color value for text_color', 'ms-inc' ) );
	}
	...
	$alt = apply_filters( 'ms_placeholder_image_alt', __( 'WebDevStudios Placeholder Image', 'ms-inc' ) );
 ...
 * @return string
 */
function ms_get_placeholder_unsplash( $args = array() ) {
	$default_args = array(
		'width'				=> '',
		'height'			=> '',
		'category'			=> '',
